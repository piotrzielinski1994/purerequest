...
export type BrunoExportWriter = {
  save: (files: BrunoFileMap, suggestedName: string) => Promise<boolean>;
};

type BrunoWriterDeps = {
  pickDir: () => Promise<string | null>;
  writeFile: (path: string, content: string) => Promise<void>;
};

export function createBrunoWriter(deps: BrunoWriterDeps): BrunoExportWriter {
  return {
...
export function createTauriBrunoWriter(): BrunoExportWriter {
  return createBrunoWriter({
    pickDir: () =>
...
export function createNoopBrunoWriter(): BrunoExportWriter {
  return {
