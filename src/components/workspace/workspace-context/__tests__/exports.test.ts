...
// sonner is the observable boundary: exports fire a global toast() on
// success/failure, so assert on the mocked calls.
vi.mock("sonner", () => ({
...
function toastFired(message: string): boolean {
  return mockToast.mock.calls.some((c) => String(c[0]) === message);
}
...
describe("createExports - exportBruno routing (AC-012)", () => {
  ...
  it("should export the selected folder as the collection root and toast on success", async () => {
    ...
    await vi.waitFor(() =>
      expect(toastFired("Exported Bruno collection")).toBe(true),
    );
  });
  ...
  it("should show an error toast if the writer save rejects", async () => {
    ...
    await vi.waitFor(() =>
      expect(toastFired("Failed to export Bruno collection")).toBe(true),
    );
  });
});
...
describe("createExports - exportPostman routing (AC-013)", () => {
  ...
  it("should export the selected folder as the collection root and toast on success", async () => {
    ...
    await vi.waitFor(() =>
      expect(toastFired("Exported Postman collection")).toBe(true),
    );
  });
  ...
  it("should show an error toast if the Postman writer save rejects", async () => {
    ...
    await vi.waitFor(() =>
      expect(toastFired("Failed to export Postman collection")).toBe(true),
    );
  });
});
...
describe("createExports - exportOpenapi routing (AC-012)", () => {
  ...
  it("should export the selected folder as the document root and toast on success", async () => {
    ...
    await vi.waitFor(() =>
      expect(toastFired("Exported OpenAPI document")).toBe(true),
    );
  });
  ...
  it("should show an error toast if the OpenAPI writer save rejects", async () => {
    ...
    await vi.waitFor(() =>
      expect(toastFired("Failed to export OpenAPI document")).toBe(true),
    );
  });
});
