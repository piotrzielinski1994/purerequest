...
Mirror dbui's CodeMirror find path verbatim (no grid to port). A `search()` extension plus a
...
- CREATE `src/components/workspace/find-bar.tsx` - shared presentational find bar (copy dbui).
- CREATE `src/components/workspace/editor-find.tsx` - CM search panel rendering FindBar + `editorFind(openKey)` (copy dbui).
- CREATE `src/lib/shortcuts/to-codemirror-key.ts` - "Mod+F"->"Mod-f" bridge (copy dbui).
